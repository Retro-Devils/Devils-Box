...
function main_menu() {
  ...
  if [ -f /usr/local/bin/box ] && [ -f /usr/local/bin/Devils-Box ]; then 
    ...
    choice=(dialog --backtitle "$BACKTITLE" --title "MAIN MENU " \
      ...
      --menu "DEVILS BOX--------------RETRODECK V1.0" 23 50 30 )
    if [ "$DB_STATUS" == 1 ]; then
      options=( \
      - "<---->Downloaders<------>"
      1 "Console Packs"
...
bash "$HOME"/Devils-Box/scripts/RETRODECK/RETRODECK-Packs.sh
fi
}
...
bash "$HOME"/Devils-Box/scripts/RETRODECK/RETRODECK-Homebrews.sh
fi
}
...
bash "$HOME"/Devils-Box/scripts/RETRODECK/RETRODECK-PickChoose.sh
fi
}
...
bash "$HOME"/Devils-Box/scripts/RETRODECK-Translations.sh
fi
}
...
clear
  ...
  sudo rm "$HOME"/RetroPie/retrodeckmenu/Devils-Box.sh
...
Offline ... Downloads not Availible Please Connect To Internet!" 0 0
  ...
  git reset --hard
  git pull
  cp "$HOME"/Devils-Box/Devils-Box.sh -f "$HOME"/RetroPie/retrodeckmenu/
  ...
  chmod 755 "$HOME"/RetroPie/retrodeckmenu/Devils-Box.sh
  sleep 1
  if [ -d "$HOME/RetroPie/retrodeckmenu/Devils-Box/" ]; then sudo rm -fR "$HOME"/RetroPie/retrodeckmenu/Devils-Box/; fi
  bash "$HOME"/RetroPie/retrodeckmenu/Devils-Box.sh
  exit 1
fi
}
