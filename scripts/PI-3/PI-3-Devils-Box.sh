...
function main_menu() {
  ...
  if [ -f /usr/local/bin/box ] && [ -f /usr/local/bin/Devils-Box ]; then 
    ...
    choice=(dialog --backtitle "$BACKTITLE" --title "MAIN MENU " \
      ...
      --menu "DEVILS BOX--------------PI3 V1.0" 23 50 30 )
    if [ "$DB_STATUS" == 1 ]; then
      options=( \
      - "<---->Downloaders<------>"
      1 "Artwork Packs"
...
bash "$HOME"/Devils-Box/scripts/PI-3/PI-3-Artwork.sh
fi
}
...
bash "$HOME"/Devils-Box/scripts/PI-3/PI-3-Packs.sh
fi
}
...
bash "$HOME"/Devils-Box/scripts/PI-3/PI-3-Homebrews.sh
fi
}
...
bash "$HOME"/Devils-Box/scripts/PI-3/PI-3-PickChoose.sh
fi
}
...
bash "$HOME"/Devils-Box/scripts/PI-3/PI-3-Translations.sh
fi
}
